...
export default function SMSLogsList() {
  ...
  const smsCampaign = campaigns?.filter((campaign) => campaign.type === 'SMS');
  const notFound = !smsCampaign.length;
  const deleteCampaign = useRemoveCampaign();
  const assignCampaignDialog = useBoolean();

  const handleViewRow = useCallback(
    (id: number) => {
      push(paths.dashboard.general.campaigns.smsLogDetail(id));
    },
    [push]
  );
          ...
          <Scrollbar>
            <Table size={table.dense ? 'small' : 'medium'} sx={{ minWidth: 960 }}>
              <TableHeadCustom
                ...
                rowCount={smsCampaign?.length}
                numSelected={table.selected.length}
                onSort={table.onSort}
                onSelectAllRows={(checked) =>
                  table.onSelectAllRows(
                    checked,
                    smsCampaign?.map((row: ICampaignItem) => String(row.id))
                  )
                }
              />
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={denseHeight}
                  emptyRows={emptyRows(table?.page, table?.rowsPerPage, smsCampaign?.length || 0)}
                />
